...
class PenguinOptionPage {
            	...
            	<div id="penguin-header">
                	<div class="penguin-logo-container">
                        <div id="penguin-custom-logo">
                            <a title="<?php echo $this->page_title; ?>" <?php echo ($this->page_logo_url != "") ? 'href="'.$this->page_logo_url.'"' : "";?>><img width="50" height="50" src="<?php echo $this->page_logo?>" alt="logo"></a>
                            <div>
	...
	function addInputText($item,$bool = false){
		...
		if($bool ){
			if(isset($item['min'])){
				$extra .= ' min="'.intval($item['min']).'"';
			}
			if(isset($item['max'])){
				$extra .= ' max="'.intval($item['max']).'"';
			}
		}
		?>
			<input id="<?php echo $this->option_name . $item['property']; ?>" name="<?php echo $this->option_name . '[' . $item['property'] . ']'; ?>" value="<?php echo esc_html($this->getCurrentValue($item)); ?>" class="penguin-input-text <?php echo $bool ? " penguin-input-text-number" : ""; ?>" type="<?php echo $bool ? "number" : "text"; ?>" <?php echo $extra; ?>>
		<?php
	}
