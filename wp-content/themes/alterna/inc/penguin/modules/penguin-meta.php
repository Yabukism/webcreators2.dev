				...
				switch($field['type']){
					case 'field':
						if(count($field['default']) > 0){
							foreach($field['default'] as $custom_field){
								if (isset($_REQUEST[$custom_field['name']])) {
									update_post_meta($post_id, $custom_field['name'], $_REQUEST[$custom_field['name']]);
								}
							}
						}
...
class PenguinMetaPage {
	...
	function PenguinMetaPage($page_obj = array()) {
		...
		foreach($this->page_elements as $elements){
			foreach($elements['fields'] as $element){
				array_push($this->fields, array('name'=>$element['name'],'type'=>$element['type'],'default'=> (isset($element['default']) ? $element['default'] : '')));
			}
		}
			...
			echo '<div id="'.$elements['id'].'" class="penguin-meta-content">';
				...
				foreach($elements['fields'] as $element){
					...
					switch($element['type']){
						...
						case 'field': $this->addFixedField($element);break;
	...
	function addFixedField($item){
		$data_count = count($item['default']);
		$fields_count	= count($item['fileds']);
		if($data_count < 1 || $fields_count < 2){	
			return;	
		}
	?>
    	<div class="penguin-custom-fileds <?php echo 'penguin-custom-fileds-count-'.$fields_count; ?>">
				...
				?>
                    <ul class="penguin-custom-element">
						...
						foreach($fileds as $filed){ 
							if($field_count >= $fields_count - 1){break;}
						?>
                    		<li class="penguin-custom-element-filed"><input type="text" readonly="readonly" value="<?php echo esc_html($filed); ?>"></li>
						<?php 
							$field_count++;
						} 
						?>
                        <li class="penguin-custom-element-filed"><input type="text" name="<?php echo esc_attr($list['name']); ?>" value="<?php echo esc_html($this->getDefaultData($list)); ?>"></li>
                    </ul>
                <?php
				}}
				?>
