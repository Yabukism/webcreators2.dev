...
<?php
	...
	public function handle_essb_save_metabox() {
		global $post, $post_id, $essb_available_button_positions;
	...
	public function import_fanscounter() {
		...
		$defaults = ESSBSocialFansCounterHelper::default_options();
	...
	public function restore_settings() {
		...
		$backup_string = isset($backup_element['configuration']) ? $backup_element['configuration'] : '';
		if ($backup_string != '') {
	...
	public function apply_readymade() {
		...
		$ready_made_string = isset($user_options['ready_made']) ? $user_options['ready_made'] : '';
		...
		if ($ready_made_string != '') {
			...
			if (!empty($exist_setting)) {
				$new_options = EasySocialShareButtons3::comvert_ready_made_option($exist_setting);
