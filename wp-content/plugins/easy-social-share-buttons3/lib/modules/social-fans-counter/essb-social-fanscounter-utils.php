...
class ESSBSocialFansCounterUtils {
	...
	public static function css_bg_class($social) {
		...
		if (!empty($template)) {
			...
			if ($template == "grey-transparent" || $template == "color-transparent" || $template == "lite") {
	...
	public static function css_text_color_class() {
		...
		$template = self::get_option('template');
		if (!empty($template)) {
			if ($template == "grey" || $template == "color") {
	...
	public static function css_total_class($total_type = 'button') {
