...
class ESSBSocialFansCounterUpdater {
	...
	private static function facebook_page() {
		...
		$request = wp_remote_get ( 'https://graph.facebook.com/v2.2/' . ESSBSocialFansCounterHelper::get_option ( 'facebook.id' ) . '?access_token=' . ESSBSocialFansCounterHelper::get_option ( 'facebook.access_token' ) );
	...
	public static function vk_community() {
		...
		$result = 0;
		try {
			$data = @self::essb_remote_get( "http://api.vk.com/method/groups.getById?gid=$id&fields=members_count");
