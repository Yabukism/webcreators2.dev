...
class ESSBCore {
	...
	public function register_assets() {
		...
		// mobile CSS load
		if ($this->is_mobile() && (in_array('sharebar', $this->general_options['button_position']) || 
	...
	public function load() {
		...
		$this->general_options['deactivate_homepage'] = ESSBOptionValuesHelper::options_value($this->options, 'deactivate_homepage');
	...
	function display_sharebottom() {
		if (!$this->is_mobile()) { return; }
		$post_types = $this->general_options['display_in_types'];
		
		if ($this->check_applicability($post_types, 'sharebottom')) {
			printf('<div class="essb-mobile-sharebottom">%1$s</div>', $this->generate_share_buttons('sharebottom'));
		}
	}
	
	function display_sharebar() {
		if (!$this->is_mobile()) {
			return;
		}
		$post_types = $this->general_options['display_in_types'];
		
		if ($this->check_applicability($post_types, 'sharebar')) {
			$mobile_sharebar_text = ESSBOptionValuesHelper::options_bool_value($this->options, 'mobile_sharebar_text');
			if ($mobile_sharebar_text == "") { $mobile_sharebar_text = "Share this"; }
			
			echo '<div class="essb-mobile-sharebar" onclick="essb_mobile_sharebar_open();">';
			printf ('<div class="essb-mobile-sharebar-icon"></div><div class="essb-mobile-sharebar-text">%1$s</div>', $mobile_sharebar_text);
			echo '</div>';
			
			echo '<div class="essb-mobile-sharebar-window">';
			echo '<div class="essb-mobile-sharebar-window-close-title" onclick="essb_mobile_sharebar_close(); return false;">';
			echo '<a href="#" class="essb-mobile-sharebar-window-close" ></a>';
			echo '</div>';
			echo '<div class="essb-mobile-sharebar-window-content">';
			echo $this->generate_share_buttons('sharebar');
			echo '</div>';
			echo '<div class="essb-mobile-sharebar-window-shadow"></div>';
		}
	}
	
	
	function display_sharepoint() {
		if (!$this->is_mobile()) {
			return;
		}
		$post_types = $this->general_options['display_in_types'];
	
		if ($this->check_applicability($post_types, 'sharepoint')) {
				
			echo '<div class="essb-mobile-sharepoint" onclick="essb_mobile_sharebar_open();">';
			echo '<div class="essb-mobile-sharepoint-icon"></div>';
			echo '</div>';
			...
			echo '</div>';
			echo '<div class="essb-mobile-sharebar-window-content">';
			echo $this->generate_share_buttons('sharepoint');
			echo '</div>';
			echo '<div class="essb-mobile-sharebar-window-shadow"></div>';
		}
	}
		...
		if ($is_valid) {
			...
			if (!$bottombar_content_area) {
				...
				if ($bottombar_content_area_pos == "left") {
					...
					$output .= $bottombar_usercontent;
					$output .= '</div>';
					$output .= sprintf('<div class="essb_bottombar_inner_buttons essb_topbar_align_%1$s">', $bottombar_buttons_align);
					$output .= $ssbuttons;
					$output .= '</div>';
				}
				else {
	...
	function display_popup($is_shortcode = true, $shortcode_popafter = '', $shortcode_options = array(), $share_options = array()) {
		...
		if ($is_valid) {
			...
			if ($popup_window_title != '') {
				$output .= sprintf('<h3>%1$s</h3>', $popup_window_title);
			}
			if ($popup_user_message != '') {
				$output .= sprintf('<div class="essb-popup-content-message">%1$s</div>', $popup_user_message);
			}
	...
	function display_flyin($is_shortcode = false, $shortcode_options = array(), $share_options = array()) {
		...
		if ($is_valid) {
			...
			if ($flyin_window_title != '') {
				$output .= sprintf('<h3>%1$s</h3>', $flyin_window_title);
			}
			if ($flyin_user_message != '') {
				$output .= sprintf('<div class="essb-flyin-content-message">%1$s</div>', $flyin_user_message);
			}
	...
	function shortcode_display_postfloat($shortcode_options = array(), $share_options = array()) {
		...
		return $this->generate_share_buttons('postfloat', 'share', $share_options, true, $shortcode_options);
	}
	...
	function display_postfloat($content) {
		...
		$post_types = $this->general_options['display_in_types'];
		if ($this->check_applicability($post_types, 'postfloat')) {
			$links_before = $this->generate_share_buttons('postfloat');
			$this->resource_builder->add_js(ESSBResourceBuilderSnippets::js_build_generate_postfloat_reveal_code(), true, 'essb-postfloat-code');
		}
	...
	function display_excerpt($content) {
		...
		if ($this->check_applicability($post_types, 'excerpt')) {
	...
	function display_inline($content) {
		...
		if ($content_position == "content_float" || $content_position == "content_floatboth") {
			if ($this->check_applicability($post_types, 'float')) {
				$links_before = $this->generate_share_buttons('float');
			}
		}
	...
	function get_post_share_details() {
		...
		return array("url" => $url, "title" => $title, "image" => $image, "description" => $description, "twitter_user" => $twitter_user,
				"twitter_hashtags" => $twitter_hashtags, "twitter_tweet" => $twitter_customtweet, "post_id" => $post->ID, "user_image_url" => "", "title_plain" => $title_plain, 'short_url_whatsapp' => '', 'short_url_twitter' => '', 'short_url' => '');
	}
	...
	function actions_update_post_count() {
		...
		die(json_econde(array(" post_id ".$post_id.", service = ".$service_id.", current_value = ".$current_value)));
	}
	...
	function essb_shortcode_profiles($atts) {
		...
		return ESSBSocialProfiles::generate_social_profile_icons($sc_network_address, $sc_button_type, $sc_button_size, $sc_button_fill, $sc_nospace);
	}
		...
		foreach ( $networks as $network ) {
			...
			if ($network == "pinterest_pin") {
				if (ESSBOptionValuesHelper::unified_true ( $pinterest_pin )) {
					...
					$output .= ESSBNativeButtonsHelper::draw_single_native_button('pinterets', $native_button_options, $counters_bool, $skinned_buttons);
	...
	function essb_shortcode_total_shares($atts) {
		...
		$output = "";
		$output .= '<div class="essb-total '.$css_class_align.'" data-network-list="'.$buttons.'" data-url="'.$data_url.'" data-full-number="'.$data_full_number.'" data-post="'.$data_post_id.'">';
