...
class ESSBNativeButtonsHelper {
	...
	public static function native_button_defaults() {
		...
		$defaults['pinterest_text'] = $pinterest_text;
	...
	public static function draw_native_buttons($settings = array(), $active_buttons = array(), $counters = false, $same_line = false, $skinned_buttons = false) {
		...
		if (!$same_line) {
			...
			if (isset($settings['message_like_buttons']) && !empty($settings['message_like_buttons'])) {
				$settings ['message_like_buttons'] = preg_replace(array('#%title%#', '#%siteurl%#', '#%permalink%#', '#%image%#', '#%shorturl%#'), array($settings['title'], get_site_url(), $settings['url'], $settings['image'], $settings['short_url']), $settings ['message_share_buttons']);
				$output .= sprintf ( '<div class="essb_message_above_like">%1$s</div>', stripslashes ( $style ['message_like_buttons'] ) );
			}
	...
	public static function pinterest_button_code($settings = array(), $counters = false) {
		...
		$pinterest_text = isset($settings['pinterest_text']) ? $settings['pinterest_text'] : '';
