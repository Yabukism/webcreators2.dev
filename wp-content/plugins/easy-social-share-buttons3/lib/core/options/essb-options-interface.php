...
class ESSBOptionsInterface {
	...
	public static function draw_form_start() {
		...
		echo '<div class="essb-options" id="essb-options">';
	}
	...
	public static function draw_content($options = array()) {
		...
		foreach($options as $section => $fields) {
			...
			foreach ($section_options as $option) {
				ESSBOptionsFramework::draw_options_field($option);
			}
