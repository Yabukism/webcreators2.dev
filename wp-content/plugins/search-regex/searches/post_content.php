...
class SearchPostContent extends Search
{
	function find ($pattern, $limit, $offset, $orderby)
	{
		...
		$results = array ();
		$posts   = $wpdb->get_results ( "SELECT ID, post_content, post_title FROM {$wpdb->posts} WHERE post_status != 'inherit' AND post_type IN ('post','page','portfolio') ORDER BY ID $orderby" );
